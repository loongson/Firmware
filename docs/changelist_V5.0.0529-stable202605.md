...
其中，本次同时发布的基于 V5.0.0529-stable202605 的公测版本（构建标识 20260623174016_51a17aab），主要用于兼容社区主线 Linux 内核中已有的动态调频调压及睿频 (Boost) 功能。该构建标识中，20260623174016 为构建时间戳，51a17aab 为对应构建所使用代码/补丁状态的短标识。带有该构建标识的固件在系统中显示的 BIOS 版本仍为 V5.0.0529-stable202605。

对于开发者，如需进一步核对实际刷入固件的详细版本信息，可通过 `dmidecode -t bios` 查看 BIOS Version、Release Date 等字段，例如：

```text
Version: Loongson-FwSdk-V5.0.0529-stable202605
Release Date: 06/23/26 17:45:38
```

注：`Release Date` 字段来源于实际 BIOS 构建信息，受构建流程影响，可能与构建标识中的时间戳存在分钟级差异。
